# =============================================================================
# BlueLock Crypto Tracking V1.4 - Database Module
# =============================================================================
...
def get_watchlist() -> list[dict]:
    """Return all watchlist entries (symbol, notes, added_at), ordered by symbol."""
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT symbol, notes, added_at FROM watchlist ORDER BY symbol"
        ).fetchall()
    return [dict(row) for row in rows]


def add_to_watchlist(symbol: str, notes: str = "") -> None:
    """Add a coin to the watchlist (ignores if already present)."""
    with get_connection() as conn:
        conn.execute(
...
def update_watchlist_note(symbol: str, notes: str) -> bool:
    """
    Update the note text for a watched coin.
    Returns True if updated, False if the symbol isn't being watched.
    """
    with get_connection() as conn:
        cursor = conn.execute(
